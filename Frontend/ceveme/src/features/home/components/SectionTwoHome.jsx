  ...
  const features = [
    {
      ...
      description: "Nasza zaawansowana sztuczna inteligencja analizuje każdą ofertę pracy i dopasowuje Twoje doświadczenie do wymagań rekrutera.",
      color: "from-kraft to-bookcloth"
    },
    {
      icon: <Target className="w-8 h-8" />,
      title: "Dopasowanie do ATS",
      description: "Generujemy CV optymalizowane pod systemy ATS, zwiększając Twoje szanse na przejście pierwszej selekcji o 95%.",
      color: "from-bookcloth to-kraft"
    },
    {
      icon: <Download className="w-8 h-8" />,
      title: "Eksport w sekundach",
      description: "Pobierz gotowe CV w formacie PDF lub DOCX, gotowe do wysłania. Profesjonalny wygląd gwarantowany.",
      color: "from-kraft to-manilla"
    },
    {
      icon: <Sparkles className="w-8 h-8" />,
      title: "Nieograniczone Warianty",
      description: "Twórz nielimitowaną liczbę wersji CV dostosowanych do różnych ofert i branż. Jedna aplikacja, nieskooczone możliwości.",
      color: "from-manilla to-kraft"
    }
  ];

  return (
    <div id="info" className="min-h-screen w-full bg-gradient-to-b from-ivorylight via-ivorymedium to-ivorydark overflow-hidden py-16">
      <div className="flex flex-col justify-center items-center px-4 text-center w-full max-w-full mb-12">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-2 mb-6 bg-kraft/10 backdrop-blur-sm rounded-full border border-kraft/20">
          ...
          <span className="text-sm font-semibold text-slatedark">Co oferujemy</span>
        </div>
        ...
        <p className="text-lg md:text-xl text-clouddark max-w-3xl mt-4">
          Kompleksowe narzędzie do zarządzania karierą zawodową z inteligentnym generatorem CV
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          {features.map((feature, index) => (
            ...
            >
              ...
              {/* Icon */}
              <div className={`relative inline-flex items-center justify-center w-16 h-16 rounded-xl bg-gradient-to-br ${feature.color} text-white mb-6 shadow-md group-hover:scale-110 transition-transform duration-300`}>
                {feature.icon}
              </div>

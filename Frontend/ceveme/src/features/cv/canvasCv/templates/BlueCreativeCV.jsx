...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
  ...
  // --- PAGE BREAK HELPER ---
  // Sprawdza czy element o wysokości h zmieści się na bieżącej stronie
  // Jeśli nie, przenosi y na początek następnej strony
  const checkPageBreak = (currentY, elementHeight, bottomMargin = 20) => {
    ...
    // Jeśli element przekroczyłby dolną granicę strony
    if (currentY + elementHeight > pageBottom) {
      // Przenieś na początek następnej strony
      const nextPageTop = (currentPage + 1) * PAGE_H + CONTENT_TOP;
      return nextPageTop;
    }
...
<svg xmlns="http://www.w3.org/2000/svg"
  ...
  for (const exp of experiences) {
    // Sprawdź czy cały blok doświadczenia zmieści się (szacowana wysokość ~25mm)
  ...
  for (const p of projects) {
    // Sprawdź czy cały blok projektu zmieści się (szacowana wysokość ~20mm)
    ly = checkPageBreak(ly, 20);
  ...
  if (edus.length) {
    ...
    for (const e of edus) {
      // Sprawdź czy cała pozycja edukacji zmieści się (szacowana wysokość ~15mm)
      ry = checkPageBreak(ry, 15);
  ...
  if (gdpr) {
    // Użyj większej z pozycji ly i ry jako punktu startowego
    const startY = Math.max(ly, ry);
    const gdprY = checkPageBreak(startY + 15, 20);
    const x = LEFT_X;
    const w = PAGE_W - 10;
    text(x, gdprY, w, gdpr, {
      ...BODY_MUTED,

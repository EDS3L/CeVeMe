...
export default function Canvas({
  ...
  activeGroupIds,
  setActiveGroupIds,
  setSelectedId, // <- "setSelectedIdOnly" z App
  updateNode,
  removeNode, // <— NOWE: przekazujemy z App
  setDocument, // <— NOWE: przekazujemy z App (do wklejania)
  pageRef,
  showGrid = false,
  onMetricsChange = () => {},
}) {
  ...
  const groupNodes = useMemo(
    () => (isGrouped ? doc.nodes.filter((n) => n && groupSet.has(n.id)) : []),
    [isGrouped, doc.nodes, groupSet]
  );
  ...
  const handleGlobalKeys = useCallback(
    (e) => {
      ...
      if (mod && e.key.toLowerCase() === 'c') {
        ...
        if (!ids.length) return;
        e.preventDefault();
        const map = new Map(doc.nodes.filter(Boolean).map((n) => [n.id, n]));
        const toCopy = ids
          .map((id) => map.get(id))
          .filter(Boolean)
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Nody */}
          {doc.nodes.filter(Boolean).map((node) => {
          ...
          {isGrouped && groupBBox && (
            <>
              <div
                onMouseDown={(e) => {
                  e.stopPropagation();
                  const any = doc.nodes.find((n) => n && groupSet.has(n.id));
                  const frames = Object.fromEntries(
                    groupNodes.map((n) => [n.id, { ...n.frame }])
                  );

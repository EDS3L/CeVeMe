...
export default function ApplicationHistoryPage() {
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    const updatedHistories = histories.map((history) =>
      history.id === id ? { ...history, status: newStatus, dateOfLastModified: history.dateOfLastModified } : history
    );
    ...
    try {
      ...
      console.error(`Nie udało się zaktualizować statusu dla aplikacji ${id}`, error);
  ...
  const filteredHistories = useMemo(() => {
    return histories.filter((h) => {
      const matchesStatus = filters.status === "ALL" ? true : h.status === filters.status;
  ...
  const renderContent = () => {
    ...
    if (view === "table") {
      return <ApplicationHistoryTable histories={filteredHistories} onStatusChange={handleStatusChange} />;
    }
    return (
      <div className="space-y-6">
        {filteredHistories.map((history) => (
          <ApplicationHistoryCard
            key={history.id}
            history={history}
            onStatusChange={(newStatus) => handleStatusChange(history.id, newStatus)}
          />
        ))}
          ...
          <header className="mb-6 sm:mb-8">
            <div className="flex flex-col items-center gap-5">
              <div className="text-center">
                <h1 className="text-4xl sm:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-700 to-teal-600">Historia Aplikacji</h1>
                <p className="text-slate-600 text-lg mt-2">Śledź i zarządzaj wszystkimi swoimi aplikacjami w jednym miejscu.</p>
              </div>
              <div className="flex flex-col items-center gap-4 w-full">
                ...
                <FiltersBar
                  filters={filters}
                  onChange={setFilters}
                  onClear={() => setFilters({ status: "ALL", from: "", to: "" })}
                  resultsCount={filteredHistories.length}
                />
                {filters.status !== "ALL" && (
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-slate-600">Wybrany status:</span>

...
export default function StatusBadge({ status, value, size = 'md' }) {
  const key = status ?? value ?? 'PENDING';
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 font-semibold rounded-full ring-1 ${style} ${sizing}`}
    >
